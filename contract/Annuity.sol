...
contract Annuity {
    ...
    function confirme(uint year, uint month, uint day) {
        ...
        //保單尚未被確認
        if(status != 0) {
            confirmeEvent(msg.sender , "保單無法重複確認", now);
        }
        else {
          //進入契約撤銷期
          status = 1;
          ...
          confirmeEvent(msg.sender , "保單確認成功", now);
        }
    }
    ...
    function revoke() {
        ...
        if(status != 1) {
            //通知保險公司合約不能撤銷
            revokeEvent(msg.sender , "不再有效撤銷期限內", now);
        }
        else {
          //撤銷契約
          status = 4;
          //通知保險公司進行契約撤銷流程
          revokeEvent(msg.sender , "撤銷成功", now);
        }
    }
